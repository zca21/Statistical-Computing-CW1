...
#Loading in dataset and setting working directory
setwd("~/Desktop/Statistical Computing/CW/Statistical-Computing-CW1")
eco.df <- read.table("ecoStudy.txt",header = T)

#Turning habitat variable into factor to R to treat it as a categorical variable for plotting and analysis
...
#Selecting 1st quantile from the density column of eco-study dataset using quantile function
...
#(ii)
table(habitat)
#For habitat A, B and C there are 100, 125 and 75 observations in each category respectively
...
#attaching dataframe again so can access new variable logDensity created
attach(eco.df)

#Saving graphic created below to pdf file called Boxplot.pdf into current directory (set at start up)
...
boxplot(density~habitat,
        main="Box plot of density in each habitat",
        xlab="Habitat type",
        ylab="Density",
...
boxplot(logDensity~habitat,
        main="Box plot of log density in each habitat",
        xlab="Habitat type",
        ylab="Log(density+1)",
        col=c("red","blue","green"),
...
F.stat.numerator <- ((100*((mean(density.A)-mean(density))^2))+
                       ...
                       (75*((mean(density.C)-mean(density))^2)))/(3-1)
...
# Part e
#Function to calculate the residuals within a  selected habitat
...
#Viewing the QQ plot the tails diverge from the line (especially the upper tail which deviates significantly) 
#and the shape is a slight U (quadratic shape). This gives evidence that the normality assumption has not been satisfied.
#Thus using the current evidence the normality assumption is invalid.
...
calcLogMix2Gamma <- function(x,alpha,beta,p){
  ...
  pdf.single<-p*(x^(alpha[1]-1)*exp(-beta[1]*x)*beta[1]^alpha[1])/gamma(alpha[1])+
    (1-p)*(x^(alpha[2]-1)*exp(-beta[2]*x)*beta[2]^alpha[2])/gamma(alpha[2])
  
  #calculating the log of the joint probability distribution
  log.joint.prob.dist <- log(prod(pdf.single))
  
  return(log.joint.prob.dist)
}
...
calcMidRiemannLoop <- function(xVec,alpha,beta,p,q,k){
  ...
  #setting up vector to store areas of each individual (x_{i-1},x_{i}) segment
  ...
  #having to split the area into 2 and calculating the area separately for area above and below x-axis
  #as in second plot (b) for interval between [a',b']
  ...
  for (i in 2:length(xVec)){
    ...
    interval.area[i-1] <- (xVec[i]-xVec[i-1])*abs(smoothCurve(x,alpha,beta,p,q,k))
  }
...
calcMidRiemann <- function(xVec,alpha,beta,p,q,k){
  ...
  x.values <-(xVec[-1]+xVec[1:length(xVec)-1])/2
  
  #calculating each intervals approximate area using equation (6) without summation
  interval.area <- interval.region*abs(smoothCurve(x.values,alpha,beta,p,q,k))
